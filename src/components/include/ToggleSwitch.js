...
function ToggleSwitch(props) {
    return (
        <div className={setClass("toggle-switch", props.class)} data-ts-color={props.color}>
            <label id={props.id + '-label'} className="ts-label">{props.title}</label>
            <input id={props.id} type="checkbox" name={props.name} checked={props.checked} disabled={props.disabled}
                onChange={(e) => {
